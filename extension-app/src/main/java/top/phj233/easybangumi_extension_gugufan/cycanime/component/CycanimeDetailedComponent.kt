...
class CycanimeDetailedComponent(private val cartoonUtil: CartoonUtil) : ComponentWrapper(), DetailedComponent {
    ...
    private fun getCartoonDetailById(id: String): Cartoon {
        val videoDetail = cartoonUtil.getCartoonDetailById("cyc",id)
        videoDetail.source = source.key
        return videoDetail
    }

    private fun getPlayLineById(string: String): List<PlayLine> {
        return cartoonUtil.getPlayLineById("cyc",string)
    }
